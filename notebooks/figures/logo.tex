...
\begin{tikzpicture}[
    ...
    \draw [->,very thick] (control) ++ (-1.5,0.4) --
        node[midway,above] {} ++ (-2.5,0) -- (actuator);
    \draw [->,very thick] (actuator) -- ++(0,1.6) -- 
        node[midway,above] {MV} ++(2.5,0);
    \draw [->,very thick] (process) ++ (1.5,-0.4) -- 
        node[midway,above] {PV} ++(2.5,0) -- (sensor);
    \draw [->,very thick] (sensor) -- ++(0,-1.6) --
        node[midway,above] {} ++ (-2.5,0);
    
    \draw [<-,very thick] (process) ++(-1.5,0.4) -- 
        node[midway,above] {DV} ++(-2,0) node[left] {}; % {Disturbance Variables};
    \draw [<-,very thick] (control)  ++(1.5,-0.4) --
        node[midway,above] {SP} ++ (2,0) node[right] {}; %  {Setpoint Variables};
    \draw [->,very thick] (process) ++ (1.5,0.4) --
        node [midway,above] {CV} ++(2,0) node[right] {}; % {Controlled Variables};
    \draw [->,very thick] (control) ++ (-1.5,-0.4) -- ++(-2,0)
        node[left] {}; % {Diagnostics};
    
    %\node at (7,3.5) {Physical Realm};
    %\node at (7,2.5) {Information Realm};
    %\node at (7,7.0) {\large Feedback Control = Sensing + Computations + Actuation};
    
    %\node[left] at (3,4.6) {Manipulated Variables};
    %\node[right] at (11,4.6) {Process (Measured) Variables};
